...
int matrix_invert(int N, double *matrix) {
    ...
    int *pivot = (int *)malloc(N*N*sizeof(int));
    ...
    dgetrf_((__CLPK_integer*)&N, (__CLPK_integer*)&N, matrix, (__CLPK_integer*)&N, (__CLPK_integer*)pivot, (__CLPK_integer*)&error);
    ...
    dgetri_((__CLPK_integer*)&N, matrix, (__CLPK_integer*)&N, (__CLPK_integer*)pivot, workspace, (__CLPK_integer*)&N, (__CLPK_integer*)&error);
...
//	 printf("\n\n");
    ...
    //cout << mGPrime.GetRows() << " " << mGPrime.GetColumns() << endl;
    
    Wml::GMatrixd mGPrimeInverse( mGPrime.GetRows(), mGPrime.GetColumns() );

    
    
//    cv::Mat img( mGPrime.GetRows(), mGPrime.GetRows(), CV_64FC1);
//    
//    int sz = mGPrime.GetRows();
//    cout << sz << endl;
//    
//    for (int i = 0; i < sz; i++){
//        for (int j = 0; j < sz; j++){
//            img.at<double>(i,j) = mGPrime(i,j);
//        }
//    }
//    
...
//    cv::Mat inverse = img.inv(cv::DECOMP_LU);
//    t2 = (cv::getTickCount()-t1)/freq;
//    std::cout << "DECOMP_CHOLESKY:" << t2 << std::endl;
//
//    for (int i=0; i < sz; i++){
//        for (int j=0; j<sz; j++){
//            //A[j * sz + i] = mGPrime(i,j);
//            mGPrimeInverse(i,j) = inverse.at<double>(i,j);
//        }
//    }
//    
    ...
    for (int i=0; i < sz; i++){
        for (int j=0; j<sz; j++){
            //A[j * sz + i] = mGPrime(i,j);
            mGPrimeInverse(i,j) = A[j * sz + i];
        }
    }
     
    float timeF = ofGetElapsedTimef() - timef;
    //cout << timeF << endl;
    
    
//    matrix <float> M1( mGPrime.GetRows(), mGPrime.GetRows());
//    
//    for (int i=0; i < M1.getactualsize(); i++){ // define random values for initial matrix
//        for (int j=0; j<M1.getactualsize(); j++){
//            M1.setvalue(i,j,mGPrime(i,j));
//        }
//    }
//    
//    M1.invert(); 
